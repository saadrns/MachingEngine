...
type lendOrderBook struct {
	// one product corresponds to one order book
	product *models.Product

	// depths: asks & bids
	depths map[models.Side]*lenddepth
...
type rateOrderIdKey struct {
	rate    decimal.Decimal
	orderId int64
}

func NewLendOrderBook(product *models.Product) *lendOrderBook {
	asks := &lenddepth{
		queue:  treemap.NewWith(priceOrderIdKeyAscComparator),
		orders: map[int64]*LendBookOrder{},
	}
	bids := &lenddepth{
	...
	orderBook := &lendOrderBook{
		product:       product,
		depths:        map[models.Side]*lenddepth{models.SideBuy: bids, models.SideSell: asks},
		orderIdWindow: newWindow(0, orderIdWindowCap),
	}
	return orderBook
}

func (o *orderBook) ApplyLendOrder(order *models.Order) (logs []Log) {
...
func (o *orderBook) CancelLendOrder(order *models.Order) (logs []Log) {
...
func (o *orderBook) LendSnapshot() orderBookSnapshot {
...
func (o *orderBook) LendRestore(snapshot *orderBookSnapshot) {
...
func (o *lendOrderBook) lendnextLogSeq() int64 {
	o.logSeq++
	return o.logSeq
}

func (o *orderBook) lendnextTradeSeq() int64 {
	o.tradeSeq++
	return o.tradeSeq
}
...
func (d *lenddepth) lendadd(order LendBookOrder) {
	d.orders[order.OrderId] = &order
	d.queue.Put(&priceOrderIdKey{order.Rate, order.OrderId}, order.OrderId)
}

func (d *lenddepth) lenddecrSize(orderId int64, size decimal.Decimal) error {
	...
	if order.Size.IsZero() {
		...
		d.queue.Remove(&rateOrderIdKey{order.Rate, order.OrderId})
	}

	return nil
}

type LendBookOrder struct {
	OrderId int64
...
func newOfferBookOrder(order *models.LendingOrder) *LendBookOrder {
	return &LendBookOrder{
		OrderId: order.Id,
		Size:    order.Size,
		Funds:   order.Funds,
		Rate:    order.Rate,
...
func newDemandBookOrder(order *models.MarginOrder) *LendBookOrder {
	return &LendBookOrder{
		OrderId: order.Id,
		Size:    order.Size,
		Funds:   order.Funds,
...
func rateOrderIdKeyAscComparator(a, b interface{}) int {
	aAsserted := a.(*rateOrderIdKey)
	bAsserted := b.(*rateOrderIdKey)

	x := aAsserted.rate.Cmp(bAsserted.rate)
	if x != 0 {
		return x
	}
...
func rateOrderIdKeyDescComparator(a, b interface{}) int {
	aAsserted := a.(*rateOrderIdKey)
	bAsserted := b.(*rateOrderIdKey)

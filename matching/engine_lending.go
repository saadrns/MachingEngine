...
type LendSnapshot struct {
	OrderBookSnapshot lendOrderBookSnapshot
	OrderOffset       int64
}

type lendOffsetOrder struct {
	Offset int64
...
// Responsible for constantly pulling orders and writing to channel
func (e *Engine) runFetcherlend() {
	var offset = e.orderOffset
	if offset > 0 {
		offset = offset + 1
	}
	...
	for {
		offset, order, err := e.orderReader.FetchOrderLend()
		if err != nil {
			logger.Error(err)
			continue
		}
		e.orderCh <- &lendoffsetOrder{offset, order}
	}
}
